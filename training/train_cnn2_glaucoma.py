...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str, default='models/checkpoints/glaucoma')
...
def run_single_lr(args, lr, use_4ch, in_channels, train_indices, val_indices,
    ...
    model     = create_cnn2_model(pretrained=True, dropout=0.4, in_channels=in_channels)
    criterion = nn.CrossEntropyLoss(weight=class_weights)
    optimizer = optim.AdamW(model.parameters(), lr=lr)
    scheduler = CosineAnnealingLR(optimizer, T_max=args.epochs)

    def save_checkpoint_fn(model, optimizer, epoch, metrics, scheduler):
        checkpoint_path = get_checkpoint_filename(f'cnn2_glaucoma_{lr_tag}', checkpoint_dir)
    ...
    history = trainer.train(save_checkpoint_fn=save_checkpoint_fn,
                            model_name=f'cnn2_glaucoma_{lr_tag}')
...
def main():
    ...
    logger = setup_logging(log_dir=f'{args.output_dir}/logs',
                           log_filename='train_cnn2_glaucoma_sweep.log')

    log_hyperparameters(logger, {
        'model': 'CNN-2 Glaucoma — LR Sweep',
    ...
    train_dataset_full = REFUGEDataset(
        labels_file=args.labels_file, transform=train_transform,
        use_4ch=use_4ch, split='test'
    )
    val_dataset_full = REFUGEDataset(
    ...
    logger.info("\n" + "="*60)
    logger.info("LR SWEEP RESULTS — CNN-2 Glaucoma")
    logger.info("="*60)
