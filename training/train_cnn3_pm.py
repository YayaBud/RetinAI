...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str, default='models/checkpoints/pm')
...
def run_single_lr(args, lr, use_4ch, in_channels, train_indices, val_indices,
    ...
    class_weights = compute_class_weights(train_dataset, num_classes=2, device=args.device)
    logger.info(f"Class weights: {class_weights.cpu().tolist()}")

    model     = create_cnn3_model(pretrained=True, dropout=0.4, in_channels=in_channels)
    criterion = nn.CrossEntropyLoss(weight=class_weights)
    optimizer = optim.AdamW(model.parameters(), lr=lr)
    scheduler = CosineAnnealingLR(optimizer, T_max=args.epochs)

    def save_checkpoint_fn(model, optimizer, epoch, metrics, scheduler):
        checkpoint_path = get_checkpoint_filename(f'cnn3_pm_{lr_tag}', checkpoint_dir)
...
def main():
    ...
    logger = setup_logging(log_dir=f'{args.output_dir}/logs',
                           log_filename='train_cnn3_pm_sweep.log')

    log_hyperparameters(logger, {
        'model': 'CNN-3 Pathologic Myopia — LR Sweep',
    ...
    logger.info("\n" + "="*60)
    logger.info("LR SWEEP RESULTS — CNN-3 Pathologic Myopia")
    logger.info("="*60)
