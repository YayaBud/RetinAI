...
export default function StatsSection() {
  ...
  return (
    <section
      ...
      style={{ zIndex: 10, backgroundColor: 'rgba(8,15,23,0.55)' }}
    >
      <div className="max-w-[1100px] mx-auto px-6 lg:px-12">
        ...
        <div ref={metricsRef} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div
              key={i}
              className="bg-[#0d1a28] rounded-2xl p-8 card-shadow text-center border border-white/5"
            >
              <div className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center mx-auto mb-4">
                <metric.icon className="w-7 h-7 text-white/50" />
              </div>
